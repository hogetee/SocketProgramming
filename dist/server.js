...
class ChatServer {
    ...
    buildPhotoPayload(entry, options) {
        return {
    ...
    handleDeleteCommand(context, tokens) {
        if (!context.name) {
            return;
        }
        if (tokens.length < 2) {
    ...
    formatHistoryEntry(entry, requester) {
        ...
        switch (entry.type) {
            ...
            case "group":
                return `[${timestamp}] [Group:${entry.group ?? "unknown"} #${entry.id}] ${entry.sender ?? "unknown"}: ${body}`;
            case "system":
            default:
